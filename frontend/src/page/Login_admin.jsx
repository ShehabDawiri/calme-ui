...
export default function Login_admin() {
  ...
  const fetshuser = async () => {
    let newErrors = {};
    try {
      const response = await axios.get(`http://127.0.0.1:5000/doctor/${formData.user_email}`);
        
       if(response.status==202){
        navigate("/SessionNotes");
       }
        else{ newErrors.exsist = "the admin not  exsist ";
            setErrors(newErrors);}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2 mt-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
